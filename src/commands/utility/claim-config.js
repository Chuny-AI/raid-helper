...
const { createErrorEmbed, createSuccessEmbed, createInfoEmbed, createPremiumEmbed } = require('../../utils/errorEmbeds');
    ...
    try {
      ...
      if (!botPermissions.has(['ViewChannel', 'SendMessages', 'EmbedLinks'])) {
        const errorEmbed = createErrorEmbed(
          ...
          'El bot no tiene permisos suficientes en este canal para funcionar correctamente.'
        );
        return await interaction.editReply({
          embeds: [errorEmbed]
        });
