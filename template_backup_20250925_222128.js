...
module.exports = {
  ...
  async handleAddNewWeaponGroup(interaction, sessionId) {
    try {
      ...
      // Crear una sesión temporal de creación para nuevo grupo con ID más corto
      const shortId = Date.now().toString(36); // Base36 para acortar
      ...
      templateCreationSessions.set(tempSessionId, {
        ...
        step: 'category_selection',
        data: {
          weapons: {} // Objeto vacío inicialmente, compatible con showWeaponCategorySelection
        },
        currentGroup: {
          categories: []
        },
        originalSessionId: sessionId, // Para poder volver al editor
        isNewGroup: true
      });

      // Mostrar selección de categorías para nuevo grupo
      const { showWeaponCategorySelection } = require('../../lib/template/template-create-handlers');
